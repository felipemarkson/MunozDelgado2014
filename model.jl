...
JuMP.@variable(model, 0 <= f̃ₛᵣₜ[s=Ωᴺ, Ωₛ[s], T])
JuMP.@variable(model, 0 <= gᵖₛₖₜᵦ[p=P, Ωᵖ[p], Kᵖ[p], T, B])
# JuMP.@variable(model, 0 <= gᵗʳₛₖₜᵦ[tr=TR, Ωᴺ, Kᵗʳ[tr], T, B])
JuMP.@variable(model, 0 <= g̃ˢˢₛₜ[Ωˢˢ, T])
JuMP.@variable(model, 0 <= vₛₜᵦ[Ωᴺ, T, B], start=Vbase)
